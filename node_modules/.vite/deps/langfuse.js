...
var ChatPromptClient = class _ChatPromptClient extends BasePromptClient {
  ...
  getLangchainPrompt(options) {
    const messagesWithPlaceholdersReplaced = [];
    const placeholderValues = (options == null ? void 0 : options.placeholders) ?? {};
    for (const item of this.prompt) {
      if ("type" in item && item.type === ChatMessageType.Placeholder) {
        const placeholderValue = placeholderValues[item.name];
        if (Array.isArray(placeholderValue) && placeholderValue.length > 0 && placeholderValue.every((msg) => typeof msg === "object" && "role" in msg && "content" in msg)) {
...
function safeSetTimeout(fn, timeout) {
  ...
  (t == null ? void 0 : t.unref) && (t == null ? void 0 : t.unref());
  return t;
}
function getEnv(key) {
...
var LangfuseCoreStateless = class {
  constructor(params) {
    ...
    this.baseUrl = removeTrailingSlash((options == null ? void 0 : options.baseUrl) || "https://cloud.langfuse.com");
    this.additionalHeaders = (options == null ? void 0 : options.additionalHeaders) || {};
    this.flushAt = (options == null ? void 0 : options.flushAt) ? Math.max(options == null ? void 0 : options.flushAt, 1) : 15;
    this.flushInterval = (options == null ? void 0 : options.flushInterval) ?? 1e4;
    this.release = (options == null ? void 0 : options.release) ?? getEnv("LANGFUSE_RELEASE") ?? getCommonReleaseEnvs() ?? void 0;
    this.mask = options == null ? void 0 : options.mask;
    this.sampleRate = (options == null ? void 0 : options.sampleRate) ?? (getEnv("LANGFUSE_SAMPLE_RATE") ? Number(getEnv("LANGFUSE_SAMPLE_RATE")) : void 0);
    if (this.sampleRate) {
      this._events.emit("debug", `Langfuse trace sampling enabled with sampleRate ${this.sampleRate}.`);
    }
    this.environment = (options == null ? void 0 : options.environment) ?? getEnv("LANGFUSE_TRACING_ENVIRONMENT");
    ...
    this._retryOptions = {
      retryCount: (options == null ? void 0 : options.fetchRetryCount) ?? 3,
      retryDelay: (options == null ? void 0 : options.fetchRetryDelay) ?? 3e3,
      retryCheck: isLangfuseFetchError
    };
    this.requestTimeout = (options == null ? void 0 : options.requestTimeout) ?? 5e3;
    this.sdkIntegration = (options == null ? void 0 : options.sdkIntegration) ?? "DEFAULT";
  ...
  debug(enabled = true) {
    var _a2;
    (_a2 = this.removeDebugCallback) == null ? void 0 : _a2.call(this);
  ...
  flush(callback) {
    ...
    const done = (err) => {
      ...
      callback == null ? void 0 : callback(err, items);
    ...
    if (this.isLocalEventExportEnabled && this.projectId) {
      if (!this.localEventExportMap.has(this.projectId)) {
        ...
        (_a2 = this.localEventExportMap.get(this.projectId)) == null ? void 0 : _a2.push(...items);
      }
      done();
      return;
    }
  ...
  async fetchWithRetry(url, options, retryOptions) {
    AbortSignal.timeout ?? (AbortSignal.timeout = function timeout(ms) {
  ...
  async getDataset(name, options) {
    ...
    while (true) {
      const itemsResponse = await this._getDatasetItems({
        datasetName: name,
        limit: (options == null ? void 0 : options.fetchItemsPageSize) ?? 50,
        page
      });
    ...
    if (!cachedPrompt || (options == null ? void 0 : options.cacheTtlSeconds) === 0) {
      try {
        return await this._fetchPromptAndUpdateCache({
          ...
          fetchTimeout: options == null ? void 0 : options.fetchTimeoutMs
        });
      } catch (err) {
...
var Langfuse = class extends LangfuseCore {
  constructor(params) {
    ...
    if (typeof window !== "undefined" && "Deno" in window === false) {
      this._storageKey = (params == null ? void 0 : params.persistence_name) ? `lf_${params.persistence_name}` : `lf_${langfuseConfig.publicKey}_langfuse`;
      this._storage = getStorage((params == null ? void 0 : params.persistence) || "localStorage", window);
    } else {
...
var parseChunk = (rawChunk) => {
  ...
  const chunkData = (_a2 = _chunk == null ? void 0 : _chunk.choices) == null ? void 0 : _a2[0];
  try {
    ...
    if ("delta" in chunkData) {
      return {
        isToolCall,
        data: ((_b = chunkData.delta) == null ? void 0 : _b.content) || ""
      };
    }
...
var getToolCallOutput = (toolCallChunks) => {
  ...
  for (const toolCall of toolCallChunks) {
    name = ((_a2 = toolCall.function) == null ? void 0 : _a2.name) || name;
...
var wrapMethod = (tracedMethod, config, ...args) => {
  ...
  let observationData = {
    ...
    name: config == null ? void 0 : config.generationName,
    startTime: /* @__PURE__ */ new Date(),
    promptName: (_a2 = config == null ? void 0 : config.langfusePrompt) == null ? void 0 : _a2.name,
    promptVersion: (_b = config == null ? void 0 : config.langfusePrompt) == null ? void 0 : _b.version,
  ...
  if (hasUserProvidedParent) {
    ...
    observationData = {
      ...filteredConfig,
      ...observationData,
      promptName: (config == null ? void 0 : config.promptName) ?? ((_c = config == null ? void 0 : config.langfusePrompt) == null ? void 0 : _c.name),
      // Maintain backward compatibility for users who use promptName
      promptVersion: (config == null ? void 0 : config.promptVersion) ?? ((_d = config == null ? void 0 : config.langfusePrompt) == null ? void 0 : _d.version)
      // Maintain backward compatibility for users who use promptVersion
    };
  } else {
    const langfuse = LangfuseSingleton.getInstance(config == null ? void 0 : config.clientInitParams);
    langfuseParent = langfuse.trace({
      ...config,
      ...observationData,
      id: config == null ? void 0 : config.traceId,
...
var observeOpenAI = (sdk, langfuseConfig) => {
  return new Proxy(sdk, {
    get(wrappedSdk, propKey, proxy) {
      ...
      const defaultGenerationName = `${(_a2 = sdk.constructor) == null ? void 0 : _a2.name}.${propKey.toString()}`;
      const generationName = (langfuseConfig == null ? void 0 : langfuseConfig.generationName) ?? defaultGenerationName;
